...
`AI Studio` 当前采用的是一个 **uv workspace 风格的 Python monorepo**，由 6 个子包组成，目标是复用 `Umber Studio` 的分层思想，但保持更低复杂度。
...
- `sandbox -> core`
- `serve -> core`
- `app -> core/ext/client/serve/sandbox`
...
- package 分层方式
- 单向依赖方向
- `core / ext / serve / app / client / sandbox` 的职责划分
