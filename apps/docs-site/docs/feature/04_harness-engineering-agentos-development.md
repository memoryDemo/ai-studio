...
这篇文档不是 OpenAI Harness Engineering 原文翻译，而是结合 OpenAI、Martin Fowler、LangChain 等资料后，对 `Meyo` 开发方法的落地解释。请按三层阅读：

| 类型 | 含义 | 本文位置 |
|---|---|---|
| 官方/来源事实 | OpenAI 提出 Harness Engineering；LangChain 将 agent harness 拆成 prompts、tools、skills、sandbox、orchestration、hooks 等组件；Fowler/Böckeler 用 guides/sensors 和 feedback/feedforward 解释 coding agent harness | 第 1-4 节的基础概念 |
| 本文归纳 | Prompt / Context / Harness 三层关系，workflow、skill、multi-agent 的边界 | 第 2-4 节、第 10-11 节 |
| Meyo 建议 | 针对当前 `Meyo` 壳项目的最小 Harness、AGENTS.md 模板、playbook、验证门、阶段路线 | 第 5 节之后 |

本文中关于 “Meyo 应该怎么做” 的内容是项目落地建议，不是 OpenAI、LangChain 或 Fowler 的官方要求。
