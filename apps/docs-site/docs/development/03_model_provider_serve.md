...
> 这一节记录模型服务后端的开发顺序：先把配置和模型抽象立住，再迁移远程 LLM provider，最后用 worker manager 把模型服务跑起来
...
在 LLM provider 目录上，这次已经把远程大语言模型供应商目录完整迁移过来。当前后端可以扫描识别这些 provider：
...
具体供应商不应该直接散落在 worker 或 API 里。它们要挂在统一的 provider/proxy 模型体系下。
...
这时底层配置、参数、接口、provider 基础层都已经有了，具体供应商只需要作为 provider 挂进去。

开发顺序是：

```text
先用 SiliconFlow 打通 LLM / embedding / rerank
-> 再迁移完整 LLM provider 目录
-> 再补各 provider 的公开配置模板
-> 最后补必要 SDK 依赖和扫描验证
```
...
- 非 OpenAI-compatible provider SDK 依赖
- provider 默认 API base
- 从环境变量读取密钥
- SiliconFlow embedding / rerank 默认验证链路

相关文件：

```text
packages/meyo-core/src/meyo/model/proxy/llms/
...
如果先写具体供应商文件，代码会到处依赖临时结构。先把基础层做好，再接 provider，代码边界会清楚很多。
...
- `base`：后端基础启动能力，不绑定具体供应商。
- `siliconflow`：硅基流动 LLM 需要的 OpenAI-compatible SDK。
- `pg_milvus_neo4j`：PostgreSQL、Milvus、Neo4j 对应驱动。
...
当前默认配置仍然启用 SiliconFlow 远程 provider。`fschat_adapter.py`、HF、vLLM、MLX、llama.cpp 这些本地模型适配器是模型运行层的预留能力，不是默认启动链路。只跑远程 proxy provider 时不需要安装 `fastchat` 和 `torch`。

完整 LLM provider 目录迁移后，可以用扫描结果确认当前可识别供应商：

```text
proxy/aimlapi, proxy/baichuan, proxy/burncloud, proxy/claude, proxy/deepseek,
proxy/gemini, proxy/gitee, proxy/infiniai, proxy/minimax, proxy/moonshot,
proxy/ollama, proxy/openai, proxy/siliconflow, proxy/spark, proxy/tongyi,
proxy/volcengine, proxy/wenxin, proxy/yi, proxy/zhipu
```

## 12. 后续开发原则

后面继续接其它模型供应商或补 embedding / rerank 能力时，按同样顺序开发：
...
今天这批代码只服务一个目标：让 Meyo 后端可以从配置启动并管理远程模型 provider。

默认配置先跑 SiliconFlow，是因为它同时覆盖 LLM、embedding、rerank，适合做端到端验证；LLM provider 目录已经完整迁移，后续切换 OpenAI、DeepSeek、通义、火山、Claude、Gemini 等供应商时，不需要重写 worker manager。
...
- `packages/meyo-core/src/meyo/model/proxy/llms/aimlapi.py`：AIML API 大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/baichuan.py`：百川大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/burncloud.py`：BurnCloud 大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/chatgpt.py`：兼容协议的大语言模型客户端实现，为远程供应商提供基础调用能力。
- `packages/meyo-core/src/meyo/model/proxy/llms/claude.py`：Claude 大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/deepseek.py`：DeepSeek 大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/gemini.py`：Gemini 大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/gitee.py`：Gitee AI 大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/infiniai.py`：InfiniAI 大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/minimax.py`：MiniMax 大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/moonshot.py`：Moonshot 大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/ollama.py`：Ollama 代理模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/proxy_model.py`：代理模型请求解析和通用调用逻辑，供具体大语言模型供应商复用。
- `packages/meyo-core/src/meyo/model/proxy/llms/siliconflow.py`：硅基流动大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/spark.py`：讯飞星火大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/tongyi.py`：通义千问大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/volcengine.py`：火山方舟大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/wenxin.py`：文心一言大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/yi.py`：零一万物大语言模型供应商实现。
- `packages/meyo-core/src/meyo/model/proxy/llms/zhipu.py`：智谱大语言模型供应商实现。
...
- `packages/meyo-ext/src/meyo_ext/rag/embeddings/tongyi.py`：通义千问向量生成供应商实现。
...
- `configs/meyo-proxy-baichuan.toml`：百川代理模型配置模板。
- `configs/meyo-proxy-burncloud.toml`：BurnCloud 代理模型配置模板。
- `configs/meyo-proxy-claude.toml`：Claude 代理模型配置模板。
- `configs/meyo-proxy-deepseek.toml`：DeepSeek 代理模型配置模板。
- `configs/meyo-proxy-gemini.toml`：Gemini 代理模型配置模板。
- `configs/meyo-proxy-gitee.toml`：Gitee AI 代理模型配置模板。
- `configs/meyo-proxy-infiniai.toml`：InfiniAI 代理模型配置模板。
- `configs/meyo-proxy-minimax.toml`：MiniMax 代理模型配置模板。
- `configs/meyo-proxy-moonshot.toml`：Moonshot 代理模型配置模板。
...
- `configs/meyo-proxy-spark.toml`：讯飞星火代理模型配置模板。
- `configs/meyo-proxy-tongyi.toml`：通义千问代理模型配置模板。
- `configs/meyo-proxy-volcengine.toml`：火山方舟代理模型配置模板。
- `configs/meyo-proxy-wenxin.toml`：文心一言代理模型配置模板。
- `configs/meyo-proxy-yi.toml`：零一万物代理模型配置模板。
- `configs/meyo-proxy-zhipu.toml`：智谱代理模型配置模板。
...
- `packages/meyo-core/pyproject.toml`：补模型服务运行所需依赖，以及 OpenAI、Claude、Gemini、Ollama 等 proxy provider 可选依赖。
- `packages/meyo-app/pyproject.toml`：只保留 workspace 包关系，并提供 `base`、`siliconflow`、`multi_proxy`、`pg_milvus_neo4j` 等按需安装组合。
- `packages/meyo-ext/pyproject.toml`：补通义向量模型所需 DashScope 可选依赖。
...
-> meyo.model.proxy.llms.siliconflow 或其它具体 LLM provider
