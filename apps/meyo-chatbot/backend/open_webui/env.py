...
WEBUI_NAME = os.environ.get('WEBUI_NAME', 'Meyo')
...
if FROM_INIT_PY:
    try:
        PACKAGE_DATA = {'version': importlib.metadata.version('meyo-chatbot')}
...
if FROM_INIT_PY and 'FRONTEND_BUILD_DIR' not in os.environ:
    source_build_dir = BASE_DIR / 'build'
    packaged_frontend_dir = OPEN_WEBUI_DIR / 'frontend'
    FRONTEND_BUILD_DIR = (source_build_dir if source_build_dir.exists() else packaged_frontend_dir).resolve()
