...
export default function DeploymentPhaseContent({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>{t("deployments.choice.selectDeployment")}</DialogTitle>
        <DialogDescription>
          {selectedProvider
            ? t("deployments.choice.deploymentsOnProvider", {
                name: selectedProvider.name,
              })
            : t("deployments.choice.selectDeploymentDescription")}
        </DialogDescription>
      </DialogHeader>
              ...
              >
                ...
                <span className="text-xs text-muted-foreground">
                  {t("deployments.choice.deploymentType", {
                    type: t(`deployments.type.${deployment.type}`),
                  })}
                </span>
              </Label>
            </div>
          ...
          <div className="flex items-center gap-3 rounded-lg border p-3">
            ...
            >
              <span className="text-sm font-medium">
                {t("deployments.choice.createNewDeployment")}
              </span>
              <span className="text-xs text-muted-foreground">
                {selectedProvider
                  ? t("deployments.choice.newDeploymentOnProvider", {
                      name: selectedProvider.name,
                    })
                  : t("deployments.choice.newDeploymentDescription")}
              </span>
            </Label>
          </div>
        </RadioGroup>
