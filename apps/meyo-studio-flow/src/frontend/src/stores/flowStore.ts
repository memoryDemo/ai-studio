...
const useFlowStore = create<FlowStoreType>((set, get) => ({
  ...
  resetFlow: (flow) => {
    ...
    set({
      dismissedNodes: getStoredStringArray(`dismiss_${flow?.id}`),
      dismissedNodesLegacy: getStoredStringArray(`dismiss_legacy_${flow?.id}`),
    });
  ...
  inspectionPanelVisible: ENABLE_INSPECTION_PANEL
    ? safeGetLocalStorage("inspectionPanelVisible") !== null
      ? safeGetLocalStorage("inspectionPanelVisible") === "true"
      : true
    : false,
  setInspectionPanelVisible: (visible: boolean) => {
    if (!ENABLE_INSPECTION_PANEL) return;
    safeSetLocalStorage("inspectionPanelVisible", String(visible));
