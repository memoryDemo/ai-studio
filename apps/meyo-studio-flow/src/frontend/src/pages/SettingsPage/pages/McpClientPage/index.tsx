  ...
  switch (agent) {
    case "bob":
      return {
        ...
        steps: [
          t("mcpClient.bob.stepOpenSettings"),
          t("mcpClient.bob.stepEditGlobal"),
          t("mcpClient.stepPasteJson"),
        ],
      };
    case "claude-code":
      return {
        configPath: "",
        steps: [
          t("mcpClient.claude.stepRunCommand"),
          t("mcpClient.claude.stepManualJson"),
        ],
      };
  }
}
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-start gap-2 rounded-md bg-accent-amber px-3 py-2 text-sm text-accent-amber-foreground">
            ...
            />
            <span>
              {t("mcpClient.apiKeyNoticeBefore")}{" "}
              <code className="font-semibold">login</code>{" "}
              {t("mcpClient.apiKeyNoticeAfterLogin")}{" "}
              <Link
                to="/settings/api-keys"
                className="underline hover:text-foreground"
              >
                {t("mcpClient.apiKeysSettings")}
              </Link>
              .
            </span>
          </div>
