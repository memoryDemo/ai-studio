...
export default function ShareModal({
  ...
  async function handleGetNames() {
    ...
    await getStoreComponents({
      ...
      res?.results?.forEach((element: StoreComponentNameResult) => {
  ...
  const handleShareComponent = async (update = false) => {
    ...
    function successShare() {
      ...
      setSuccessData({
        title: t(is_component ? "share.componentShared" : "share.flowShared"),
      });
    }
    ...
    if (!update)
      saveFlowStore(
        ...
        setErrorData({
          title: t(
            is_component
              ? "share.errorSharingComponent"
              : "share.errorSharingFlow",
          ),
    ...
    return (
      <>
        <ConfirmationModal
          ...
          title={t("share.replace")}
          cancelText={t("common.cancel")}
          confirmationText={t("share.replace")}
          size={"x-small"}
          icon={"SaveAll"}
          index={6}
          onConfirm={() => {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <span className="pr-2">{t("share.title")}</span>
          <IconComponent
            name="Share3"
          ...
          {open && (
            <>
              ...
              <div className="mb-2 mt-5 flex items-center space-x-2">
                ...
                >
                  {t("share.setPublicStatus", { type: nameComponent })}
                </label>
              </div>
              <span className="text-xs text-destructive">
                <b>{t("share.attention")}:</b>{" "}
                {t("share.apiKeysRemovedOnShare")}
              </span>
            </>
          )}
        </BaseModal.Content>
        ...
        <BaseModal.Footer
          submit={{
            label: t(is_component ? "share.shareComponent" : "share.shareFlow"),
