...
export default function UpdateComponentModal({
  ...
  const columnDefs: ColDef[] = [
    { field: "id", hide: true },
    {
      headerName: t("updateComponent.component"),
      field: "display_name",
    ...
    {
      headerName: t("updateComponent.updateType"),
  ...
  return (
    ...
    >
      ...
      <BaseModal.Header>
        <span className="">
          {isMultiple
            ? t("updateComponent.updateComponents")
            : t("updateComponent.updateSingle", {
      ...
      <BaseModal.Content overflowHidden>
        <div className="flex flex-col gap-6">
          <div className="flex flex-col gap-3 text-sm text-muted-foreground">
            {isMultiple ? (
              <p>
                {t("updateComponent.multipleDescriptionPrefix")}{" "}
                <span className="font-semibold text-accent-amber-foreground">
                  {t("updateComponent.breakingLower")}
                </span>{" "}
                {t("updateComponent.multipleDescriptionSuffix")}
              </p>
            ) : (
              <>
                <p>
                  {t("updateComponent.singleDescriptionPrefix")}{" "}
                  <span className="font-semibold text-accent-amber-foreground">
                    {t("updateComponent.disconnectThisComponent")}
                  </span>
                  {t("updateComponent.singleDescriptionSuffix")}
                </p>
                <p>{t("updateComponent.sidebarLatestVersion")}</p>
              </>
            )}
          </div>
          {isMultiple && (
      ...
      <BaseModal.Footer
        submit={{
          label:
            components.length > 1
              ? t("updateComponent.updateComponents")
              : t("updateComponent.updateComponent"),
