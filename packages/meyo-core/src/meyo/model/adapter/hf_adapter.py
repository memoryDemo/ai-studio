...
from meyo.model.adapter.model_metadata import (
...
class NewHFChatModelAdapter(LLMModelAdapter, ABC):
    ...
    def get_generate_stream_function(
        ...
        """获取对应数据。"""
        from meyo.model.llm.llm_out.hf_chat_llm import huggingface_chat_generate_stream
    ...
    def load_media(self, params: Dict, messages: List[ModelMessage]):
        """加载数据或资源。"""
        from meyo.model.utils.media_utils import parse_messages
...
# 代码说明。
# 代码说明。
register_model_adapter(CommonModelAdapter)  # 代码说明。
