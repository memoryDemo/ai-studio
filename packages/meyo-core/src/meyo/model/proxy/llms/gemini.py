...
from meyo.core.interface.message import parse_model_messages
...
def _transform_to_gemini_messages(
    ...
    参考：https://github.com/GoogleCloudPlatform/generative-ai/blob/main/gemini/getting-started/intro_gemini_python.ipynb

    参数：
        messages (List[ModelMessage])：消息列表。

    返回：
        Tuple[str, List[Dict[str, Any]]]：用户问题和 Gemini 历史消息。
