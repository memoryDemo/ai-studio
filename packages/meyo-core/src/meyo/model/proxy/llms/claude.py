...
from meyo.model.proxy.llms.chatgpt import OpenAICompatibleDeployModelParameters
...
class ClaudeProxyTokenizer(ProxyTokenizer):
    ...
    async def count_token_async(self, model_name: str, prompts: List[str]) -> List[int]:
        ...
        这里依赖 Claude beta API，部分用户可能无法使用。
        """
        from meyo.util.chat_util import run_async_tasks
