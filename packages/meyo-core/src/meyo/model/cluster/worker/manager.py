...
from meyo.configs.model_config import LOGDIR
...
from meyo.model.base import ModelInstance, WorkerApplyOutput, WorkerSupportedModel
...
from meyo.model.cluster.manager_base import (
    WorkerManager,
    WorkerManagerFactory,
    WorkerRunData,
)
from meyo.model.cluster.registry import ModelRegistry
from meyo.model.cluster.storage import ModelStorage, ModelStorageItem
...
from meyo.model.utils.llm_utils import list_supported_models
from meyo.util.fastapi import create_app, register_event_handler
from meyo.util.parameter_utils import (
    ParameterDescription,
    _get_dict_from_obj,
)
from meyo.util.system_utils import get_system_info
from meyo.util.tracer import SpanType, SpanTypeRunName, initialize_tracer, root_tracer
from meyo.util.tracer.tracer_impl import TracerParameters
...
def _setup_fastapi(
    ...
    if worker_params.standalone:
        from meyo.model.cluster.controller.controller import initialize_controller
...
def _create_local_model_manager(
    ...
    from meyo.util.net_utils import _get_ip_address
    ...
    if not worker_params.register or not worker_params.controller_addr:
        ...
        from meyo.model.cluster.controller.controller import ModelRegistryClient
...
def _build_worker(
    ...
    if worker_class:
        ...
        if worker_type is None or worker_type == WorkerType.LLM:
            ...
            from meyo.model.cluster.worker.embedding_worker import (
    ...
    **kwargs,
    ...
    setup_logging(
        ...
        default_logger_filename=os.path.join(
            LOGDIR, "meyo_model_worker_manager.log"
        ),
    )
    ...
    trace_file = trace_config.file or os.path.join(
        "logs", "meyo_model_worker_manager_tracer.jsonl"
    )
    initialize_tracer(
        trace_file,
        system_app=system_app,
        root_operation_name=trace_config.root_operation_name or "MEYO-ModelWorker",
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="MEYO Model Worker")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
