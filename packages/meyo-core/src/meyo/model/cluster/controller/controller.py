...
from meyo.model.cluster.registry import EmbeddedModelRegistry, ModelRegistry
from meyo.model.parameter import DBModelRegistryParameters, ModelControllerParameters
from meyo.util.api_utils import APIMixin
from meyo.util.api_utils import _api_remote as api_remote
from meyo.util.api_utils import _sync_api_remote as sync_api_remote
from meyo.util.fastapi import create_app
...
def initialize_controller(
    ...
    if app:
        ...
        trace_file = trace_config.file or os.path.join(
            "logs", "meyo_model_controller_tracer.jsonl"
        )
        initialize_tracer(
            trace_file,
            system_app=system_app,
            root_operation_name=trace_config.root_operation_name
            or "MEYO-ModelController",
...
async def api_register_instance(request: ModelInstance):
    with root_tracer.start_span(
        "meyo.model.controller.register_instance", metadata=request.to_dict()
    ):
...
async def api_deregister_instance(
    ...
    with root_tracer.start_span(
        "meyo.model.controller.deregister_instance", metadata=instance.to_dict()
    ):
...
def _create_registry(controller_params: ModelControllerParameters) -> ModelRegistry:
    ...
    if not controller_params.registry:
        ...
        from meyo.datasource.rdbms.base import (
            RDBMSConnector,
            RDBMSDatasourceParameters,
        )
        from meyo.model.cluster.registry_impl.storage import StorageModelRegistry
        ...
        if isinstance(db_config, SQLiteConnectorParameters):
            ...
            raise ValueError(
                "MEYO only supports SQLite, MySQL and OceanBase databases as metadata "
                "storage database"
            )
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="MEYO API Server")
    parser.add_argument(
        "-c",
        "--config",
        type=str,
...
def run_model_controller(config_file: str):
    from meyo.configs.model_config import LOGDIR, ROOT_PATH
    from meyo.util.configure import ConfigurationManager
    from meyo_serve.datasource.manages.connector_manager import ConnectorManager
    ...
    setup_logging(
        ...
        default_logger_filename=os.path.join(
            LOGDIR, "meyo_model_controller.log"
        ),
    )
