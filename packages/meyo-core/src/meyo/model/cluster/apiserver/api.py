...
from meyo.core import ModelOutput
from meyo.core.interface.message import ModelMessage
from meyo.core.schema.api import (
...
from meyo.model.parameter import ModelAPIServerParameters, WorkerType
from meyo.util.chat_util import transform_to_sse
from meyo.util.fastapi import create_app
from meyo.util.tracer import initialize_tracer, root_tracer, trace
...
class APIServer(BaseComponent):
    ...
    async def get_available_models(self) -> ModelList:
        ...
        for m in models:
            model_cards.append(
                ModelCard(
                    id=m, root=m, owned_by="MEYO", permission=[ModelPermission()]
                )
            )
    ...
    async def embeddings_generate(
        ...
        with root_tracer.start_span(
            "meyo.model.apiserver.generate_embeddings",
...
async def create_completion(
    ...
    trace_kwargs = {
        "operation_name": "meyo.model.apiserver.create_completion",
...
async def create_relevance(
    ...
    with root_tracer.start_span(
        "meyo.model.apiserver.generate_relevance",
        metadata={
            "model": request.model,
            "query": request.query,
            "worker_type": WorkerType.RERANKER.value,
...
def _initialize_all(controller_addr: str, system_app: SystemApp):
    ...
    from meyo.model.cluster.worker.manager import _DefaultWorkerManagerFactory
...
def initialize_apiserver(
    ...
    setup_logging(
        ...
        default_logger_filename=os.path.join(
            LOGDIR, "meyo_model_apiserver.log"
        ),
    )
    ...
    initialize_tracer(
        ...
        root_operation_name=trace_config.root_operation_name or "MEYO-APIServer",
