...
from meyo.component import ComponentType, SystemApp
...
from meyo.util.tracer.span_storage import MemorySpanStorage
...
class TracerParameters(BaseParameters):
    ...
    file: Optional[str] = field(
        ...
        metadata={
            "help": _(
                "The file to store the tracer, e.g. meyo_webserver_tracer.jsonl"
            ),
        },
    )
...
def initialize_tracer(
    tracer_filename: str,
    root_operation_name: str = "MEYO-Webserver",
    system_app: Optional[SystemApp] = None,
    create_system_app: bool = False,
    tracer_parameters: Optional[TracerParameters] = None,
):
    """初始化并启动相关能力。"""
    from meyo.util.tracer.span_storage import FileSpanStorage, SpanStorageContainer
    ...
    if tracer_parameters and tracer_parameters.exporter == "telemetry":
        from meyo.util.tracer.opentelemetry import OpenTelemetrySpanStorage
    ...
    if system_app.app:
        from meyo.util.tracer.tracer_middleware import TraceIDMiddleware
