#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""配置基础能力，提供环境变量替换、路径解析和参数对象构造的通用实现。
"""
...
from meyo.util.singleton import Singleton

if TYPE_CHECKING:
    from meyo.component import SystemApp
    from meyo.datasource.manages import ConnectorManager
...
class Config(metaclass=Singleton):
    ...
    def __init__(self) -> None:
        ...
        self.MEYO_WEBSERVER_PORT = int(
            os.getenv(
                "MEYO_WEBSERVER_PORT",
                os.getenv("UMBER_STUDIO_WEBSERVER_PORT", 5670),
            )
        )
        self.UMBER_STUDIO_WEBSERVER_PORT = self.MEYO_WEBSERVER_PORT
        ...
        from meyo.core._private.prompt_registry import PromptTemplateRegistry
        ...
        self.LOCAL_DB_NAME = os.getenv("LOCAL_DB_NAME", "meyo")
        ...
        # 日志级别配置。
        self.MEYO_LOG_LEVEL = os.getenv(
            "MEYO_LOG_LEVEL", os.getenv("UMBER_STUDIO_LOG_LEVEL", "INFO")
        )
        self.UMBER_STUDIO_LOG_LEVEL = self.MEYO_LOG_LEVEL
        ...
        # 非流式场景配置。
        self.MEYO_APP_SCENE_NON_STREAMING_RETRIES_BASE = int(
            os.getenv(
                "MEYO_APP_SCENE_NON_STREAMING_RETRIES_BASE",
                os.getenv("UMBER_STUDIO_APP_SCENE_NON_STREAMING_RETRIES_BASE", 1),
            )
        )
        self.UMBER_STUDIO_APP_SCENE_NON_STREAMING_RETRIES_BASE = (
            self.MEYO_APP_SCENE_NON_STREAMING_RETRIES_BASE
        )
        # 非流式场景配置。
        self.MEYO_APP_SCENE_NON_STREAMING_PARALLELISM_BASE = int(
            os.getenv(
                "MEYO_APP_SCENE_NON_STREAMING_PARALLELISM_BASE",
                os.getenv(
                    "UMBER_STUDIO_APP_SCENE_NON_STREAMING_PARALLELISM_BASE", 1
                ),
            )
        )
        self.UMBER_STUDIO_APP_SCENE_NON_STREAMING_PARALLELISM_BASE = (
            self.MEYO_APP_SCENE_NON_STREAMING_PARALLELISM_BASE
        )
        # 实验性的财报模型配置。
    ...
    def local_db_manager(self) -> "ConnectorManager":
        from meyo_serve.datasource.manages import ConnectorManager
