...
"""模型服务配置常量和默认路径定义，集中管理开发配置、日志和静态资源位置。
"""
...
def _detect_root_path() -> str:
    ...
    home = os.environ.get(
        "MEYO_HOME",
        os.environ.get("UMBER_STUDIO_HOME", os.path.expanduser("~/.meyo")),
    )
...
LOGDIR = os.getenv(
    "MEYO_LOG_DIR",
    os.getenv("UMBER_STUDIO_LOG_DIR", os.path.join(ROOT_PATH, "logs")),
)
...
def _detect_skills_dir() -> str:
    ...
    env_dir = os.environ.get(
        "MEYO_SKILLS_DIR", os.environ.get("UMBER_STUDIO_SKILLS_DIR")
    )
    if env_dir:
        return env_dir
